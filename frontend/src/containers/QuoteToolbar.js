...
import { getDatabaseContext } from 'folder-ui/lib/tools'

import {
  quoteEditorSettings
} from '../actions'

export class QuoteToolbarContainer extends Component {
  render() {
    return (    
      <QuoteToolbar {...this.props} />
    )
  }
}
...
function mapStateToProps(s, ownProps) {
  ...
  return {
    ...
    deleting,
    message,
    adding:false,//state.app.quoteEditorSettings.adding ? true : false,
    getState:() => s
  }
}
...
function mapDispatchToProps(dispatch, ownProps) {
  ...
  return {
    ...
    addButtonClicked:() => {
      dispatch(quoteEditorSettings('adding', true))
    },
    closeAddMenu:() => {
      dispatch(quoteEditorSettings('adding', false))
    }
